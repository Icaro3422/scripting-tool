...
interface Video {
  ...
  status: string;
  project: { id: string; title: string; contentStyle: string | null };
  scripts: Script[];
...
export default function VideoEditorPage() {
  ...
  async function handleGenerateThumbnail() {
    const titleText = (thumbTitle.trim() || video?.title) ?? "";
    if (!titleText) {
      setThumbError("Escribe un título o usa el del video.");
      return;
    }
    ...
    if (
      ...
      setThumbError(
        "Modo local requiere un navegador con File System Access (Chrome/Edge). Usa modo Nube o otro navegador."
      );
      return;
    }
    if (storageMode === "local" && typeof window !== "undefined") {
      const win = window as unknown as {
        __scriptingToolDirHandle?: FileSystemDirectoryHandle;
      ...
      if (!win.__scriptingToolDirHandle) {
        try {
          ...
          if ((e as Error).name === "AbortError") {
            setThumbError(
              "Selecciona la carpeta donde guardar las miniaturas (ej. /Volumes/B/Downloads/scripting) para que el archivo se guarde en disco."
            );
            return;
          }
    ...
    try {
      ...
      if (data.imageBase64 && storageMode === "local") {
        ...
        const win =
          typeof window !== "undefined"
            ? (window as unknown as { __scriptingToolDirHandle?: FileSystemDirectoryHandle })
            : undefined;
        ...
        if (handle) {
          try {
            ...
            await w.write(buf);
            await w.close();
            await setLocalThumbPath(thumbnail.id, `${projectId}/${fileName}`, fileName);
          } catch (e) {
          ...
          localSaveError =
            "Miniatura guardada en la app. Para guardarla en tu carpeta, genera otra miniatura y selecciona la carpeta cuando te la pida.";
        }
      }
      setVideo((prev) =>
        prev && thumbnail
      ...
      try {
        ...
        setError(
          res.ok
            ...
            : `Error del servidor (${res.status}). Prueba otro modelo o revisa OPENROUTER_API_KEY.`
        );
        return;
      }
      if (!res.ok) {
      ...
      if (data.imageBase64 && storageModeForScene === "local") {
        ...
        if (handle) {
          try {
            ...
            await w.write(buf);
            await w.close();
            await setLocalThumbPath(thumb.id, `${projectId}/${fileName}`, fileName);
          } catch (e) {
    ...
    <div className="p-8 max-w-4xl">
      <div className="mb-6 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-[rgb(var(--text-muted))]">
            Proyecto: {video.project.title} · Los scripts generados se guardan aquí debajo.
          </p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--bg-muted))] p-4 mb-6">
        <h3 className="text-sm font-medium text-[rgb(var(--text-primary))] mb-3">
          Resumen del video · Duración y costes
        </h3>
          ...
          {latestScript ? (
            <span>
              ...
              <strong className="text-[rgb(var(--text-primary))]">
                ~{estimatedMinutes(countWords(latestScript.content))} min
              </strong>
            </span>
          ) : (
            <span className="text-[rgb(var(--text-muted))]">
              Genera un script para ver la duración estimada.
            </span>
          )}
          <span>Scripts: {video.scripts.length}</span>
          <span>Título: {video.title ? "✓" : "—"}</span>
          <span>Descripción: {video.description ? "✓" : "—"}</span>
          <span>Miniaturas: {video.thumbnails.length}</span>
        </div>
        <p className="text-xs text-[rgb(var(--text-muted))] mt-2">
          Los costes dependen de tu proveedor. Cada generación consume créditos de tu API key.
        </p>
      </div>
          ...
          <div className="rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--bg-surface))] overflow-hidden">
            <div className="px-4 py-3 border-b border-[rgb(var(--border))] bg-[rgb(var(--bg-muted))]">
              ...
              <p className="text-xs text-[rgb(var(--text-muted))]">
                Tras generar el script, se muestra por escena. Puedes generar imagen por escena.
              </p>
            </div>
            <div className="p-4">
                ...
                <>
                  {(latestScript || generatedScript) && (
                    <div className="rounded-lg bg-[rgb(var(--bg-muted))] p-3 text-xs text-[rgb(var(--text-muted))] flex flex-wrap gap-x-4 gap-y-1 mb-4">
                      <span>
                        Versión más reciente · {latestScript?.aiModel ?? "IA"} ·{" "}
                        {latestScript
                          ? new Date(latestScript.createdAt).toLocaleString("es")
                          : "Sin guardar aún"}
                      </span>
                      <span className="text-[rgb(var(--text-secondary))]">
                        {countWords(scriptContentForFragments).toLocaleString()} palabras · ~
                        {estimatedMinutes(countWords(scriptContentForFragments))} min
                      </span>
                    </div>
                  )}
                  {sceneImageError && (
              ...
              {video.scripts.length > 1 && (
                <div className="mt-4 pt-4 border-t border-[rgb(var(--border))]">
                  <p className="text-xs text-[rgb(var(--text-muted))] mb-2">
                    Otras versiones ({video.scripts.length - 1})
                  </p>
                  <ul className="space-y-2">
                    {video.scripts.slice(1, 5).map((s) => (
                      <li key={s.id} className="text-sm text-[rgb(var(--text-secondary))] truncate">
        ...
        <div className="space-y-6">
          <div className="rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--bg-surface))] p-4">
            <p className="text-sm text-[rgb(var(--text-secondary))] mb-2">
              Título actual del video: <strong>{video.title}</strong>
            </p>
            <button
              type="button"
      ...
      {tab === "description" && (
        <div className="space-y-6">
          <div className="rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--bg-surface))] p-4">
            <p className="text-sm text-[rgb(var(--text-secondary))] mb-2">
              Descripción con formato: intro, reflexión, timestamps (capítulos) derivados del script.
            </p>
            <button
              type="button"
            ...
            {(generatedDescription || video.description) ? (
      ...
      {tab === "tags" && (
        <div className="space-y-6">
          <div className="rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--bg-surface))] p-4">
            <p className="text-sm text-[rgb(var(--text-secondary))] mb-2">
              Etiquetas con alto potencial de búsqueda para el video (SEO).
            </p>
            <button
              type="button"
            ...
            {generatedTags.length > 0 && (
              <div className="mt-4 rounded-lg bg-[rgb(var(--bg-muted))] p-4">
                <p className="text-xs font-medium text-[rgb(var(--text-muted))] mb-2">
                  Copia y pega en YouTube (máx. 500 caracteres en total). Clic en una etiqueta para
                  copiarla.
                </p>
      ...
      {tab === "thumbnail" && (
        <div className="space-y-6">
          <div className="rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--bg-surface))] p-4">
            <h3 className="text-sm font-medium text-[rgb(var(--text-primary))] mb-3">
              Generar miniatura viral con IA
            </h3>
            <p className="text-xs text-[rgb(var(--text-muted))] mb-4">
              La miniatura es clave para el CTR. Usa Nano Banana o Grok para imágenes 16:9.
            </p>
            <div className="space-y-3">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setThumbWordStyle(e.target.value as "preset" | "few" | "many")
                  }
                  className="w-full rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--bg-muted))] px-3 py-2 text-[rgb(var(--text-primary))] focus:outline-none focus:ring-2 focus:ring-[rgb(var(--accent))]"
                >
                  <option value="preset">Según preset</option>
                  <option value="few">Pocas palabras (2-4)</option>
                  <option value="many">Muchas palabras (5-8)</option>
                </select>
              </div>
              <div>
                <label className="block text-xs font-medium text-[rgb(var(--text-muted))] mb-1">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={video.title || "Ej: 5 secretos que no sabías"}
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Ej: rojo, negro, amarillo"
                  className="w-full rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--bg-muted))] px-3 py-2 text-[rgb(var(--text-primary))] placeholder:text-[rgb(var(--text-muted))] focus:outline-none focus:ring-2 focus:ring-[rgb(var(--accent))]"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-[rgb(var(--text-muted))] mb-1">
                  Estilo o referencia (opcional)
                </label>
                <input
                  type="text"
                  value={thumbReferenceHint}
                  onChange={(e) => setThumbReferenceHint(e.target.value)}
                  placeholder="Ej: expresiones exageradas, fondo oscuro"
                  className="w-full rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--bg-muted))] px-3 py-2 text-[rgb(var(--text-primary))] placeholder:text-[rgb(var(--text-muted))] focus:outline-none focus:ring-2 focus:ring-[rgb(var(--accent))]"
                />
              </div>
              {thumbError && (
            ...
            {storageMode === "local" && localFolderName && (
              <div className="mt-3 rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--bg-muted))] px-3 py-2">
                <p className="text-xs font-medium text-[rgb(var(--text-primary))] mb-0.5">
                  Carpeta de assets
                </p>
                <p className="text-xs text-[rgb(var(--text-muted))] break-all">
                  {localFolderName} → scripting-tool → {projectId}
                </p>
              </div>
            )}
          </div>
